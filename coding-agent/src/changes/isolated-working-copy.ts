...
export interface CodingWorkingCopy {
  ...
  authorizeApply(
    signal?: AbortSignal
  ): Promise<WorkingCopyApplyAuthorization | WorkingCopyApplicationResult>;
  release(): Promise<void>;
}
...
export class IsolatedWorkingCopy implements CodingWorkingCopy {
  ...
  static async open(input: {
    ...
    if (input.preChange.coverage !== 'complete')
      throw new Error('An isolated working copy requires a complete pre-change snapshot.');
    const directory = path.join(input.runtimeDirectory, 'working-copies', identity(input.workId));
    ...
    if (existing === undefined) {
      ...
      try {
        ...
        if (initialized.coverage !== 'complete' || initialized.digest !== input.preChange.digest) {
          throw new Error(
            `Working copy ${input.workId} did not reproduce its exact admitted pre-change state.`
          );
        }
      } finally {
        initializedRoot.close();
      }
    } else if (
      existing.workId !== input.workId ||
      existing.sourceId !== sourceId ||
      existing.preChangeDigest !== input.preChange.digest
    ) {
      throw new Error(`Working-copy identity conflicts with run ${input.workId}.`);
    }
    ...
    try {
      ...
      if (actual.coverage !== 'complete')
        throw new Error(
          `Working copy ${input.workId} cannot be observed completely: ${actual.causes.join(', ')}.`
        );
      const descriptor = Object.freeze({
        implementationId: IMPLEMENTATION_ID,
        workingCopyId: identity(`${input.workId}:${input.preChange.digest}`),
        workId: input.workId,
        sourceId
      });
      const preChangeCheckpointId = identity(
        `${descriptor.workingCopyId}:preChange:${input.preChange.digest}`
      );
      const preChangeWorkspace = path.join(directory, 'checkpoints', preChangeCheckpointId, 'workspace');
      const preChangeSnapshotFile = path.join(
        directory,
        'checkpoints',
      ...
      if (initializing) {
        ...
        await writeExclusiveJson(manifestPath, {
          ...
          workId: input.workId,
          sourceId,
          preChangeDigest: input.preChange.digest
        });
      } else {
        if (!(await directoryExists(preChangeWorkspace)))
          throw new Error(`Working copy ${input.workId} has no durable pre-change checkpoint.`);
        ...
        if (storedPreChange.coverage !== 'complete' || storedPreChange.digest !== input.preChange.digest) {
          throw new Error(`Working copy ${input.workId} has a conflicting pre-change checkpoint.`);
        }
      }
  ...
  async checkpoint(label: string, signal?: AbortSignal): Promise<WorkingCopyCheckpoint> {
    ...
    if (snapshot.coverage === 'complete') {
      ...
      if (!(await directoryExists(destination)))
        await restoreWorkspaceSnapshot(this.root, snapshot, destination);
      await writeJsonIfAbsent(
        path.join(this.#directory, 'checkpoints', checkpointId, 'snapshot.json'),
        snapshot
      );
    }
  ...
  async rollback(checkpointId: string, signal?: AbortSignal): Promise<WorkingCopyCheckpoint> {
    ...
    try {
      ...
      if (unsupported.length > 0)
        throw new Error(`Checkpoint rollback cannot safely represent: ${unsupported.join(', ')}.`);
      ...
      const result = await this.#workingCopyJournal.withAuthority(
        this.root,
        (authority) =>
          authority.commit(transaction, {
            ...
            transactionId,
            recoveryPayload: {
              contract: 'coding-agent.working-copy-rollback@1',
      ...
      if (restored.digest !== target.digest || restored.coverage !== 'complete')
        throw new Error('Checkpoint rollback did not reproduce the exact checkpoint.');
  ...
  async authorizeApply(
    ...
    if (diff.coverage !== 'complete')
      return notApplied(`Working copy diff is incomplete: ${diff.causes.join(', ')}.`);
    const unsupported = unsupportedApplicationChanges(this.#preChangeSnapshot, workingCopy);
    if (unsupported.length > 0)
      return notApplied(`Working-copy application cannot safely represent: ${unsupported.join(', ')}.`);
    const transactionId = `apply-${identity(`${this.descriptor.workingCopyId}:${this.#preChangeSnapshot.digest}:${workingCopy.digest}`)}`;
    const prior = await this.#applicationJournal.withAuthority(
      this.#source,
      (authority) => authority.receipt(transactionId),
      signal
    );
    ...
    if (prior === undefined) {
      ...
      if (
        currentSource.coverage !== 'complete' ||
        currentSource.digest !== this.#preChangeSnapshot.digest
      ) {
        planningFailure =
          'The source workspace changed after working-copy isolation; application was not started.';
      } else {
        applicationPlan = await planTextTransaction(
          this.root,
          this.#source,
          this.#preChangeSnapshot,
          workingCopy
        );
      }
    }
    ...
    const receiptResult = (receipt: TextTransactionReceipt | undefined) =>
      receipt === undefined
        ? undefined
    ...
    return Object.freeze({
      ...
      recovery: Object.freeze({
        ...
        authority: this.#applicationJournal.recoveryIdentity,
        reconcilerId: APPLICATION_RECONCILER_ID,
        transactionId
      }),
      start: async (startSignal: AbortSignal) => {
        ...
        if (!applicationPlan)
          return notApplied(
            planningFailure ?? 'Working-copy application has no authorized transaction plan.'
          );
        const authorizedPlan = applicationPlan;
        const result = await this.#applicationJournal.withAuthority(
          this.#source,
          (authority) =>
            authority.commit(authorizedPlan, {
              signal: startSignal,
              transactionId,
        ...
        if (application.status === 'applied') {
          ...
          if (applied.coverage !== 'complete' || applied.digest !== workingCopy.digest)
            return notApplied(
              'Application committed but the source workspace no longer matches the exact working copy.'
            );
        }
        return application;
      },
      reconcile: async (reconcileSignal: AbortSignal) => {
        const receipt = await this.#applicationJournal.withAuthority(
          this.#source,
          (authority) => authority.receipt(transactionId),
          reconcileSignal
        );
        ...
        if (result.status === 'applied') {
          ...
          if (applied.coverage !== 'complete' || applied.digest !== workingCopy.digest) {
            return Object.freeze({
              ...
              result: notApplied(
                'Application has a durable commit receipt, but the source workspace no longer matches the exact working copy.'
              )
            });
          }
        }
  ...
  #assertOpen(): void {
    if (this.#released) throw new Error('The working copy has been released.');
  }
}

export async function deleteIsolatedWorkingCopy(runtimeDirectory: string, workId: string): Promise<void> {
  await rm(path.join(runtimeDirectory, 'working-copies', identity(workId)), {
    recursive: true,
    force: true
  });
}

export function isolatedWorkingCopyWorkspacePath(runtimeDirectory: string, workId: string): string {
  return path.join(runtimeDirectory, 'working-copies', identity(workId), 'workspace');
}
...
function workspaceDiff(before: WorkspaceSnapshot, after: WorkspaceSnapshot): WorkingCopyDiff {
  ...
  const causes = [
    ...before.causes.map((cause) => `pre-change:${cause}`),
    ...after.causes.map((cause) => `working-copy:${cause}`)
  ];
  return Object.freeze({
...
function unsupportedApplicationChanges(
  ...
  for (const changedPath of changed) {
    ...
    if (initial?.kind === 'file' && initial.content !== 'text') {
      unsupported.push(`${changedPath} (binary or unbounded source file)`);
      continue;
    }
    if (final?.kind === 'file' && final.content !== 'text') {
      unsupported.push(`${changedPath} (binary or unbounded working-copy file)`);
      continue;
    }
    if (initial?.kind === 'directory' && final === undefined) {
      unsupported.push(`${changedPath} (directory removal)`);
      continue;
    }
    if (initial === undefined && final?.kind === 'directory') {
      const hasAddedFile = changed.some(
        (descendantPath) =>
          descendantPath.startsWith(`${changedPath}/`) && afterByPath.get(descendantPath)?.kind === 'file'
      );
      if (!hasAddedFile || (final.mode ?? 0) % 0o1000 !== 0o700)
        unsupported.push(`${changedPath} (empty or non-private directory addition)`);
      continue;
    }
    ...
    if (
      (initial !== undefined && initial.kind !== 'file') ||
      (final !== undefined && final.kind !== 'file')
    )
      unsupported.push(`${changedPath} (non-file replacement)`);
  }
  return Object.freeze(unsupported);
}

async function planTextTransaction(
  contentRoot: RootedFileAuthority,
  targetRoot: RootedFileAuthority,
  before: WorkspaceSnapshot,
  after: WorkspaceSnapshot
): Promise<TextPatchTransactionPlan> {
  const beforeByPath = new Map(before.entries.map((entry) => [entry.path, entry]));
  const afterByPath = new Map(after.entries.map((entry) => [entry.path, entry]));
  ...
  for (const changedPath of changedWorkspacePaths(before, after)) {
    ...
    if (final?.kind === 'file') {
      if (final.content !== 'text' || final.sha256 === undefined || final.mode === undefined)
        throw new Error(`Working-copy file cannot be applied as text: ${changedPath}`);
      const content = await readExactText(contentRoot, final);
      if (initial?.kind === 'file') {
        if (initial.sha256 === undefined || initial.bytes === undefined)
          throw new Error(`Pre-change file is incomplete: ${changedPath}`);
        const expectedIdentity = await assertExactTarget(
          targetRoot,
          changedPath,
          initial.sha256,
          initial.bytes
        ...
        writes.push(
          Object.freeze({
            ...
            mode: final.mode,
            overwrite: true,
            expectedCurrentSha256: initial.sha256,
            expectedCurrentIdentity: expectedIdentity
          })
        );
      } else {
        if ((await targetRoot.inspectPath(changedPath)).kind !== 'absent')
          throw new Error(`Target appeared before transaction staging: ${changedPath}`);
        writes.push(
          Object.freeze({
            path: changedPath,
            content,
  ...
  const parentDirsToCreate = changedWorkspacePaths(before, after)
    .filter(
      (changedPath) =>
        beforeByPath.get(changedPath) === undefined && afterByPath.get(changedPath)?.kind === 'directory'
    )
    .sort(
      (left, right) => left.split('/').length - right.split('/').length || compareCodeUnits(left, right)
    );
  return Object.freeze({
    writes: Object.freeze(writes),
    removes: Object.freeze(removes),
    parentDirsToCreate: Object.freeze(parentDirsToCreate)
  });
}
...
async function readExactText(root: RootedFileAuthority, entry: WorkspaceSnapshotEntry): Promise<string> {
  ...
  try {
    ...
    if (bytes.byteLength !== entry.bytes || sha256(bytes) !== entry.sha256)
      throw new Error(`Working copy content changed while staging the transaction: ${entry.path}`);
...
function applicationResult(
  ...
  preChange: WorkspaceSnapshot,
  workingCopy: WorkspaceSnapshot,
  diff: WorkingCopyDiff,
  transactionId: string
): WorkingCopyApplicationResult {
  return committed(result)
    ? Object.freeze({
        status: 'applied' as const,
        preChangeDigest: preChange.digest,
        workingCopyDigest: workingCopy.digest,
        changedPaths: Object.freeze(diff.entries.map((entry) => entry.path)),
        transactionId
      })
    : notApplied(
        `Working-copy application transaction ${transactionId} did not commit: ${transactionFailure(result)}.`
      );
}
function committed(result: TextTransactionResult): boolean {
  return result.outcome === 'committed' || result.outcome === 'committed_with_residue';
}
...
function notApplied(reason: string): WorkingCopyApplicationResult {
  return Object.freeze({ status: 'not_applied' as const, reason });
}
...
export async function restoreWorkspaceSnapshot(
  ...
  if (snapshot.coverage !== 'complete')
    throw new Error('Only complete workspace snapshots can be materialized.');
  ...
  try {
    ...
    for (const entry of directories)
      await mkdir(path.join(staging, ...entry.path.split('/')), { recursive: true, mode: 0o700 });
    for (const entry of snapshot.entries) {
      ...
      if (
        ...
        throw new Error(`Snapshot contains an unmaterializable entry: ${entry.path}`);
      ...
      try {
        ...
        if (content.byteLength !== entry.bytes || sha256(content) !== entry.sha256)
          throw new Error(`Source changed before materialization: ${entry.path}`);
    ...
    for (const entry of [...directories].sort(
      ...
      if (entry.mode !== undefined)
        await chmod(path.join(staging, ...entry.path.split('/')), entry.mode & 0o7777);
    }
...
async function readManifest(filePath: string): Promise<WorkingCopyManifest | undefined> {
  try {
    ...
    if (
      ...
      !digest(value.preChangeDigest)
    )
      throw new Error(`Invalid working-copy manifest: ${filePath}`);
    return Object.freeze({
      version: 1,
      workId: value.workId,
      sourceId: value.sourceId,
      preChangeDigest: value.preChangeDigest
    });
  } catch (error) {
...
async function writeExclusiveJson(filePath: string, value: unknown): Promise<void> {
  await writeFile(filePath, JSON.stringify(value), { encoding: 'utf8', flag: 'wx', mode: 0o600 });
}
...
function decodeSnapshot(value: unknown): WorkspaceSnapshot {
  if (
    ...
    !nonnegative(value.fileCount) ||
    !nonnegative(value.totalBytes)
  )
    throw new Error('Working copy checkpoint snapshot is invalid.');
  const entries = value.entries.map((entry): WorkspaceSnapshotEntry => {
    if (!record(entry) || typeof entry.path !== 'string' || !snapshotKind(entry.kind))
      throw new Error('Working copy checkpoint entry is invalid.');
    return Object.freeze({
      path: entry.path,
      kind: entry.kind,
      ...(nonnegative(entry.mode) ? { mode: entry.mode } : {}),
      ...(nonnegative(entry.bytes) ? { bytes: entry.bytes } : {}),
      ...(digest(entry.sha256) ? { sha256: entry.sha256 } : {}),
      ...(entry.content === 'text' || entry.content === 'binary' ? { content: entry.content } : {})
    });
  });
  if (createHash('sha256').update(JSON.stringify(entries)).digest('hex') !== value.digest)
    throw new Error('Working copy checkpoint digest is invalid.');
  return Object.freeze({
    digest: value.digest,
    coverage: value.coverage,
    causes: Object.freeze(value.causes.map(String)),
    entries: Object.freeze(entries),
    fileCount: value.fileCount,
    totalBytes: value.totalBytes
  });
}
function sourceIdentity(root: RootedFileAuthority): string {
  return identity(JSON.stringify(root.identity));
}
...
function safeIdentity(value: string, label: string): string {
  if (!/^[a-f0-9]{64}$/u.test(value)) throw new Error(`Invalid ${label} identity.`);
  return value;
}
function requiredLabel(value: string): string {
  const normalized = value.trim();
  if (normalized.length === 0 || normalized.length > 200)
    throw new Error('Checkpoint label must be non-empty and bounded.');
  return normalized;
}
...
function snapshotKind(value: unknown): value is WorkspaceSnapshotEntry['kind'] {
