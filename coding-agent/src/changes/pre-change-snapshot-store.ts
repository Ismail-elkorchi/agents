...
/** Returns the immutable pre-effect workspace state owned by one admitted work. */
export async function loadOrCapturePreChangeSnapshot(input: {
  ...
  const directory = preChangeSnapshotDirectory(input.workId);
  const stored = await input.state.read(`${directory}/manifest.json`);
  if (stored !== undefined) return loadPreChangeSnapshot(input.state, input.workId);
  if (input.resuming)
    throw new Error(`Pre-change snapshot for continuing work ${input.workId} is missing.`);
  ...
  if (versionControlIdentity(versionControlBeforeCapture) !== versionControlIdentity(versionControl)) {
    throw new Error(
      `Workspace version-control state changed while run pre-change snapshot ${input.workId} was being captured.`
    );
  }
...
export async function loadPreChangeSnapshot(
  ...
  if (manifestText === undefined) throw new Error(`Pre-change snapshot for run ${workId} is missing.`);
...
export async function deletePreChangeSnapshot(state: PrivateStateDirectory, workId: string): Promise<void> {
  ...
  const manifest = decodeManifest(JSON.parse(stored), workId);
...
function decodeManifest(value: unknown, workId: string): PreChangeSnapshotManifest {
  if (
    !isRecord(value) ||
    ...
    throw new Error(`Pre-change snapshot manifest for ${workId} is invalid.`);
  }
...
function decodeEntries(value: unknown): readonly WorkspaceSnapshotEntry[] {
  if (!Array.isArray(value) || value.length > ENTRIES_PER_CHUNK)
    throw new Error('Pre-change snapshot entry chunk is invalid.');
  return Object.freeze(
    value.map((entry) => {
      if (
        !isRecord(entry) ||
        Object.keys(entry).some(
          (key) => !['path', 'kind', 'mode', 'bytes', 'sha256', 'content'].includes(key)
        ) ||
...
function preChangeSnapshotDirectory(workId: string): string {
  return `work-pre-change-snapshots/${createHash('sha256').update(workId).digest('hex')}`;
}

function versionControlIdentity(value: RepositoryVersionControl): string {
  if (value.kind !== 'git') return JSON.stringify(value);
  if (value.status.kind === 'unavailable')
    return JSON.stringify({ kind: 'git', status: { kind: 'unavailable', reason: value.status.reason } });
...
function decodeVersionControl(value: unknown): RepositoryVersionControl {
  ...
  if (
    ...
    return Object.freeze({ kind: 'unavailable', reason: value.reason });
  if (
    value.kind !== 'git' ||
    Object.keys(value).some((key) => key !== 'kind' && key !== 'status') ||
    !isRecord(value.status)
  )
    throw new Error('Pre-change snapshot version-control state is invalid.');
...
function chunk<T>(items: readonly T[], size: number): readonly (readonly T[])[] {
  ...
  for (let index = 0; index < items.length; index += size)
    chunks.push(Object.freeze(items.slice(index, index + size)));
...
function isContentHazard(value: unknown): value is ContentHazard {
  return (
    ...
    value === 'bidirectional_control' ||
    value === 'invisible_unicode' ||
