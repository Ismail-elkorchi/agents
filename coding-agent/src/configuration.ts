...
export async function loadCodingAgentConfiguration(
  ...
  try {
    ...
    if (!rootedFileIdentitiesEqual(file.identity, currentIdentity))
      throw new Error(`Project configuration changed while it was read: ${canonicalPath}`);
    const text = bytes.toString('utf8');
    const adopted = security.adoptContent({
      content: text,
      kind: 'source',
      sourceUri: `file:${canonicalPath}`,
      scope: '.',
...
export function parseCodingAgentConfiguration(input: unknown): CodingAgentConfiguration {
  ...
  if (
    ...
    throw new Error('Coding Agent configuration contains unknown fields.');
  if (
    !isCodingAgentProviderId(value.provider) ||
    typeof value.model !== 'string' ||
    value.model.length === 0
  )
    throw new Error('Configuration provider/model is invalid.');
  if (!instructionArray(value.instructions))
    throw new Error('Workspace instructions must contain confined relative paths.');
  if (
    !isRecord(value.tools) ||
    Object.keys(value.tools).some((key) => key !== 'enabled') ||
    !stringArray(value.tools.enabled)
  )
    throw new Error('Project tool configuration is invalid.');
  if (
    new Set(value.tools.enabled).size !== value.tools.enabled.length ||
    !value.tools.enabled.every((tool) => CODING_AGENT_TOOLS.includes(tool))
  ) {
  ...
  if (
    ...
    Object.keys(verification).some((key) => key !== 'required' && key !== 'advisory') ||
    !checkArray(verification.required) ||
...
function checkArray(value: unknown): value is readonly CodingAgentCheckConfiguration[] {
  return (
    Array.isArray(value) &&
    value.every(
      (item) =>
...
function validLimits(value: unknown): value is CodingAgentLimitConfiguration {
  ...
  return (
    ...
    (isRecord(value.knownCost) &&
      typeof value.knownCost.amount === 'number' &&
      Number.isFinite(value.knownCost.amount) &&
      value.knownCost.amount > 0 &&
      typeof value.knownCost.currency === 'string' &&
      value.knownCost.currency.trim().length > 0)
  );
}
