...
export function codingRunUncertainties(
  ...
  if (terminal.modelOutput.status === 'indeterminate')
    uncertainties.add('The model output is indeterminate.');
  const unknownChecks = outcome.verification.checks.filter((check) => check.verdict === 'unknown');
  for (const check of unknownChecks)
    uncertainties.add(`Check ${check.id} is unknown: ${compact(check.summary)}`);
  if (outcome.verification.status === 'inconclusive' && unknownChecks.length === 0) {
    uncertainties.add('Verification is inconclusive.');
  }
  if (outcome.verification.status === 'not_run') uncertainties.add('Verification did not run.');
...
export function codingChangeUncertainties(changeReport: RunChangeReport): readonly string[] {
  const uncertainties = new Set<string>();
  for (const cause of changeReport.causes)
    uncertainties.add(`Change coverage is partial: ${humanize(cause)}.`);
  if (changeReport.facts.externalOrConcurrentPaths.length > 0) {
    uncertainties.add(
      `Change attribution is external or concurrent for: ${changeReport.facts.externalOrConcurrentPaths.join(', ')}.`
    );
  }
  for (const change of changeReport.changes) {
    if (change.conflicts.length > 0) {
      uncertainties.add(
        `Mutation receipts conflict with ${change.path}: ${change.conflicts.map(humanize).join(', ')}.`
      );
    }
  }
  return Object.freeze([...uncertainties]);
