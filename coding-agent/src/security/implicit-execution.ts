...
const explanations: Readonly<Record<ImplicitExecutionSurface, string>> = Object.freeze({
  package_lifecycle_script:
    'Package installation and task invocation may execute repository-defined lifecycle programs.',
  ...
  git_configuration:
    'Git configuration may activate aliases, includes, credential helpers, filters, and external programs.',
...
export function classifyImplicitExecution(
  surface: ImplicitExecutionSurface
): ImplicitExecutionClassification {
  return Object.freeze({ surface, kind: 'sandboxed_effect', explanation: explanations[surface] });
}

export const implicitExecutionSurfaces: readonly ImplicitExecutionSurface[] = Object.freeze(
  Object.keys(explanations) as ImplicitExecutionSurface[]
);
