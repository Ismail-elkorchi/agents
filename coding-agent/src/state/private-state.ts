...
export class PrivateStateDirectory {
  ...
  static async create(directoryPath: string): Promise<PrivateStateDirectory> {
    ...
    if (!info.isDirectory() || info.isSymbolicLink())
      throw new Error(`Private state root is not a real directory: ${absolute}`);
    ...
    if (!entries.includes('.coding-agent-state-root')) {
      if (entries.length > 0)
        throw new Error(
          `Refusing to adopt a non-empty directory without a Coding Agent state marker: ${absolute}`
        );
      let marker;
      try {
        marker = await open(
      ...
      if (marker) {
        try {
          await marker.writeFile('coding-agent-state-root-v1\n', 'utf8');
          await marker.sync();
        } finally {
          await marker.close();
        }
    ...
    try {
      ...
      if (
        !markerInfo.isFile() ||
        markerInfo.nlink !== 1 ||
        (await marker.readFile('utf8')) !== 'coding-agent-state-root-v1\n'
      )
    ...
    if (process.platform !== 'win32') {
      ...
      if ((secured.mode & 0o077) !== 0)
        throw new Error(`Private state root permissions are not private: ${absolute}`);
    }
  ...
  async read(relativePath: string): Promise<string | undefined> {
    ...
    try {
      ...
      if (!info.isFile() || info.nlink !== 1)
        throw new Error(`Private state entry is not an exclusive regular file: ${relativePath}`);
      if (info.size > 4 * 1024 * 1024)
        throw new Error(`Private state entry exceeds its read limit: ${relativePath}`);
  ...
  private target(relativePath: string): string {
    if (
      !/^[A-Za-z0-9][A-Za-z0-9._/-]{0,500}$/u.test(relativePath) ||
