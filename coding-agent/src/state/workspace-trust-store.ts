...
export class WorkspaceTrustStore {
  ...
  async read(workspace: CodingWorkspaceIdentity): Promise<WorkspaceTrustDecision | undefined> {
    ...
    const envelope = parseJsonObject(JSON.parse(encoded), {
      maxDepth: 8,
    ...
    if (envelope.version !== 1 || envelope.sha256 !== expected)
      throw new Error(`Workspace trust record is corrupt: ${workspace.id}`);
  ...
  async write(decision: WorkspaceTrustDecision): Promise<void> {
    ...
    await this.#state.write(
      this.path(decision.workspace.id),
      `${JSON.stringify({ version: 1, payload, sha256 })}\n`
    );
  }

  delete(workspace: CodingWorkspaceIdentity): Promise<void> {
    return this.#state.delete(this.path(workspace.id));
  }
  private path(workspaceId: string): string {
    return `trust/${workspaceId}.json`;
  }
}
...
function decodeDecision(value: Record<string, unknown>): WorkspaceTrustDecision {
  ...
  requireExactKeys(
    workspace,
    ['id', 'platform', 'canonicalPath', 'device', 'inode', 'mountId'],
    'workspace identity'
  );
  ...
  if (
    !trustLevel(level) ||
    (decidedBy.kind !== 'user' && decidedBy.kind !== 'organization') ||
    typeof decidedBy.subject !== 'string' ||
    typeof value.decidedAt !== 'string' ||
    typeof workspace.id !== 'string' ||
    !nodePlatform(workspace.platform) ||
    typeof workspace.canonicalPath !== 'string' ||
    typeof workspace.device !== 'string' ||
    typeof workspace.inode !== 'string' ||
    typeof workspace.mountId !== 'string'
  ) {
...
function requireExactKeys(value: Record<string, unknown>, keys: readonly string[], label: string): void {
  ...
  if (Object.keys(value).some((key) => !allowed.has(key)) || keys.some((key) => !(key in value)))
    throw new Error(`Workspace trust record has invalid ${label} fields.`);
}
