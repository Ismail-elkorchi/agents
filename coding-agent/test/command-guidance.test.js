...
test(
  'nested repository guidance precedes Sandbox preparation and the reconsidered command completes',
  {
  ...
  async (t) => {
    const command = () =>
      toolResponse('exec_command', { command: 'printf inspected', workdir: 'subproject' });
        ...
        assert(
          request.messages.some(
            (message) =>
              ...
              message.content.includes('GUIDANCE-BEFORE-PREPARATION')
          )
        );
    ...
    const fixture = await createWorkspace({
      ...
      tools: ['exec_command'],
      checks: [],
      files: { 'subproject/AGENTS.md': 'GUIDANCE-BEFORE-PREPARATION: preserve unrelated files.\n' }
    });
    let application;
    t.after(async () => {
...
test('guidance revisions require actual admitted delivery and detect additions, edits, removal and configured precedence', async (t) => {
  ...
  const { RepositoryGuidanceSession } = await import('../dist/instructions/repository-guidance.js');
  const { WorkspaceSecurityBoundary } =
    await import('../dist/security/workspace-security-boundary.js');
  ...
  await rename(path.join(directory, 'nested/AGENTS.md'), path.join(directory, 'nested/renamed.md'));
  pending = await guidance.contextPrerequisite(request);
  assert(
