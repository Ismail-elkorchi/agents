...
import { offlineCodex } from '../../application/test/helpers/codex.js';
import { createWorkspace, trust } from './fixtures/scripted-cli.js';

test('a broken response preserves committed edits and partial text through reopen, reconciliation and stop', {
  skip: process.platform !== 'linux', timeout: 30_000
}, async (t) => {
  const provider = await offlineCodex(t);
  const fixture = await createWorkspace({
    tools: ['apply_patch'], checks: [], files: { 'draft.md': 'Before.\n' }
  });
  const options = {
    root: fixture.root, stateRoot: fixture.stateRoot, providerEndpoint: provider.endpoint,
    provider: 'openai-codex', model: 'gpt-5.6-luna',
    reasoning: { strategy: 'effort', effort: 'low' }, permissionMode: 'edit'
  };
  let requests = 0;
        ...
        pull(controller) {
          if (delivered) controller.error(new Error('Injected stream disconnection'));
            ...
            controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({
              type: 'response.output_text.delta', delta: 'The draft was changed, but'
            })}\n\n`));
          }
        }
      }), { headers: { 'Content-Type': 'text/event-stream' } });
    }
    assert(requests === 1 || requests === 3, 'An uncertain provider request must not be silently repeated.');
    const output = requests === 1 ? [{
      type: 'custom_tool_call', id: 'patch-item', call_id: 'patch-call', name: 'apply_patch',
      input: '*** Begin Patch\n*** Update File: draft.md\n@@\n-Before.\n+After.\n*** End Patch'
    }] : [];
    return new Response(`data: ${JSON.stringify({ type: 'response.completed', response: {
      id: `response-${requests}`, model: body.model, status: 'completed', output,
  ...
  assert(progress.some((event) => event.type === 'assistant.interrupted' && event.content === 'The draft was changed, but'));
  const sessionId = application.state().session.sessionId;
  await application.close();

  application = await openCodingApplication({ ...options, sessionSelection: { kind: 'existing', id: sessionId } });
  await application.start();
  const restored = await application.readSession();
  assert(restored.history.entries.some((entry) => entry.type === 'assistant' && entry.content === 'The draft was changed, but'));
  assert.equal(restored.history.entries.filter((entry) => entry.type === 'tool_call' && entry.call.name === 'apply_patch').length, 1);
