...
test('loadWorkspace rejects aliased roots instead of changing workspace authority', async () => {
  ...
  await assert.rejects(
    loadWorkspace(linkRoot, { stateRoot: path.join(dir, 'state') }),
    /aliased|directory/u
  );
});
...
test('describeWorkspace remains a pure path description', () => {
  ...
  const identity = {
    id: 'workspace-' + 'a'.repeat(64),
    platform: process.platform,
    canonicalPath: path.resolve('relative-workspace'),
    device: '1',
    inode: '2',
    mountId: '3'
  };
...
test('private runtime state cannot be placed inside the workspace', async () => {
  ...
  await assert.rejects(
    loadWorkspace(root, { stateRoot: path.join(root, 'state') }),
...
test('workspace configuration validates first-party policy, checks, and exact limit names', async () => {
  ...
  assert.throws(
    () =>
      parseCodingAgentConfiguration({
        ...configuration,
        verification: {
          required: [{ id: 'same', command: 'true', coverage: 'targeted' }],
          advisory: [{ id: 'same', command: 'true', coverage: 'targeted' }]
        }
      }),
    /unique/u
  );
  assert.throws(
    () =>
      parseCodingAgentConfiguration({
        ...configuration,
        verification: { required: [{ id: 'test', command: 'npm test' }], advisory: [] }
      }),
    /Verification configuration/iu
  );
  assert.throws(
    () =>
      parseCodingAgentConfiguration({
        ...configuration,
        instructions: [{ path: 'AGENTS.md' }, { path: 'AGENTS.md' }]
      }),
    /instruction paths must be unique/u
  );
  assert.throws(
    () => parseCodingAgentConfiguration({ ...configuration, tools: { enabled: [], unknown: true } }),
    /tool configuration/iu
  );
  let accessed = false;
