...
test('coding summaries derive every remaining uncertainty from machine facts', () => {
  ...
  const report = changeReport({
    ...
    changes: [
      {
        path: 'src/app.ts',
        kind: 'modified',
        attribution: 'external_or_concurrent',
        initial: 'existing',
        preChangeVersionControl: 'changed',
        content: 'text',
        receiptSequences: [4],
    ...
    facts: {
      ...
      structuredMutationPaths: [],
      externalOrConcurrentPaths: ['src/app.ts']
    }
  });
  assert.deepEqual(
    codingRunUncertainties(
      terminal,
      report,
      testOutcome(terminal, {
        verification: {
          status: 'inconclusive',
          checks: [
            {
              ...
              verdict: 'unknown',
              summary: 'Sandbox was unavailable.',
              durationMs: 3
            }
          ]
        }
  ...
  assert.throws(
    () => codingRunUncertainties(terminal, { ...report, runId: 'other' }, testOutcome(terminal)),
    /cannot summarize/u
  );
  ...
  const partial = decodeAgentTerminalSnapshot({
    ...
    modelOutput: {
      status: 'partial',
      message: 'Only part of the task completed.',
      source: 'content',
      turnIndex: 1
    }
  });
  assert.deepEqual(codingRunUncertainties(partial, changeReport(), testOutcome(partial)), [
    'The model output is partial.'
  ]);
});
...
test('a complete model output with passed verification and exact changes has no uncertainty', () => {
  assert.deepEqual(
    codingRunUncertainties(
      ...
      changeReport(),
      testOutcome(decodeAgentTerminalSnapshot(terminalInput()))
    ),
    []
  );
});
...
function changeReport(overrides = {}) {
  ...
  return {
    ...
    facts: { changedPaths: [], structuredMutationPaths: [], externalOrConcurrentPaths: [] },
    ...overrides
  };
}
