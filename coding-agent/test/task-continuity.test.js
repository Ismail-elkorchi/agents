...
test(
  ...
  async (t) => {
    ...
    assert.deepEqual(JSON.parse(await readFile(path.join(f.root, 'settings.json'), 'utf8')), {
      port: 9090
    });
    const { history } = await app.readSession();
    assert.deepEqual(
      history.entries.filter((entry) => entry.type === 'input').map((entry) => entry.task),
      tasks
    );
    const messages = provider.chatRequests.at(-1).messages;
    for (const task of tasks)
      assert(messages.some((message) => message.role === 'user' && message.content === task));
    assert(
      messages.findIndex((message) => message.content === tasks[2]) >
        messages.findIndex((message) => message.content === 'Updated the settings.')
    );
  }
);
