...
test('Coding Agent public and persisted contracts use the shared glossary', () => {
  ...
  const prohibitedExport =
    /export\s+(?:interface|type|class|function|const)\s+([A-Za-z0-9_]*(?:Projection|Prepare|Prepared|Evaluation|Evidence|Candidate)[A-Za-z0-9_]*)/gu;
  const prohibitedPersistedTag =
    /(?:kind|type):\s*(?:z\.literal\()?['"][^'"]*(?:projection|candidate|evaluation)[^'"]*['"]/gu;
  for (const file of sources) {
    ...
    if (
      !sandboxWireAdapters.has(file) &&
      /\b(?:projection|projected|projecting|prepare|prepared|preparing|preparation|operationStatus|evidence)\b/iu.test(
        source
      )
    ) {
