...
Writing application, RPC and TUI scenarios that create or revise managed files run with the existing Linux rooted-file test group. Ubuntu CI exercises these scenarios and native namespace conformance. macOS and Windows exercise the portable history/TUI, Markdown, notes, connection, application-import and package checks; green CI does not establish managed-file support on those platforms.
