...
In the default `suggest` mode, the model can read only exact admitted managed resources and `propose_revision` is its only write capability. Its schema is compiled for the admitted operation: text intents expose only their exact intent IDs, resource IDs, anchor IDs, and replacement prose, while structural alternatives appear only for corresponding structural intents. The model does not submit paths, hashes, ranges, source preimages, or mutation authority. The operation service injects and validates those control values before appending one durable proposal; it cannot mutate user files.

Applying an accepted proposal is a separate application action using Agent Core's recoverable text transaction. The application re-derives exact preimage text from the authoritative base immediately before mutation. Required failed, unknown, stale, partially covered, or unevaluated verification blocks acceptance. Human acceptance criteria require explicit criterion-level decisions, which are retained in the editorial decision rather than inferred from a generic approval.
