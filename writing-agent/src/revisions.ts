...
import {
  ...
  type ProjectMutationSettlement
...
export async function acceptRevisionProposal(
  ...
  if (proposalView.status !== 'proposed')
    throw new Error(`Writing proposal is already ${proposalView.status}: ${input.proposalId}`);
  const proposal = proposalView.proposal;
  const operation = view.operations.get(proposal.operationId);
  if (operation === undefined)
    throw new Error(`Writing proposal operation is unavailable: ${proposal.operationId}`);
  if (proposal.baseProjectRevisionId !== view.current.revision.revisionId)
    throw new Error(`Writing proposal base is stale: ${proposal.proposalId}`);
  ...
  if (verification === undefined)
    throw new Error(
      `Writing proposal has no durable production verification through Agent Core: ${proposal.proposalId}`
    );
  const humanCriteria = new Set(
    verification.criterionCoverage
      .filter((criterion) => criterion.verificationKind === 'human')
      .map((criterion) => criterion.criterionId)
  );
  const criterionDecisions = input.humanCriterionDecisions.map((decision) =>
    humanCriterionDecisionSchema.parse(decision)
  );
  for (const decision of criterionDecisions)
    if (!humanCriteria.has(decision.criterionId))
      throw new Error(
        `Human decision targets a criterion that is not human-verified: ${decision.criterionId}`
      );
  ...
  if (!applicability.allowed)
    throw new Error(
      `Writing proposal cannot be accepted while required verification is non-passing: ${applicability.reasons.join(', ')}.`
    );
  const explanation = input.explanation.trim();
  if (explanation.length === 0)
    throw new Error('Writing proposal acceptance requires a direct-human explanation.');
  const decision = editorialDecisionSchema.parse({
    decisionId: contentId('decision', {
    ...
    proposalId: input.proposalId,
    findingIds: [
      ...verification.semanticPreservationFindings.map((finding) => finding.findingId),
      ...verification.editorialFindings.map((finding) => finding.findingId)
    ],
    criterionDecisions,
    decision: 'accepted',
    explanation,
...
export async function rejectRevisionProposal(
  ...
  if (proposalView.status !== 'proposed')
    throw new Error(`Writing proposal is already ${proposalView.status}: ${proposalId}`);
...
export async function authorizeRevisionApplication(
  project: WritingProject,
  input: {
    readonly proposalId: string;
  }
  ...
  const proposalView = view.proposals.get(input.proposalId);
  if (proposalView === undefined) throw new Error(`Unknown writing proposal: ${input.proposalId}`);
  if (proposalView.status !== 'accepted')
    throw new Error(
      `Writing apply authorization requires an accepted proposal: ${input.proposalId} is ${proposalView.status}.`
    );
  const proposal = proposalView.proposal;
  if (view.current.revision.revisionId !== proposal.baseProjectRevisionId)
    throw new Error(
      `Writing apply authorization cannot bind a stale project revision: ${proposal.proposalId}`
    );
  const verification = view.productionVerifications.get(proposal.proposalId);
  if (verification === undefined)
    throw new Error(
      `Writing apply authorization requires durable production verification: ${proposal.proposalId}`
    );
  ...
  if (!applicability.allowed)
    throw new Error(
      `Writing apply authorization cannot bind non-passing verification: ${applicability.reasons.join(', ')}.`
    );
...
const appliedWritingRevisionSchema = z
  .strictObject({
    proposalId: z.string().min(1),
    revisionId: z.string().min(1),
    transactionId: z.string().min(1),
    fileChanges: mutationSettlementSchema.unwrap().shape.oldAndNewHashes,
    provenance: z.array(authorshipProvenanceSchema).readonly(),
    recoveredFinalization: z.boolean()
  })
  .readonly();

/** Application authorization and the mutation journal are bound before shared effect dispatch. */
export async function applyRevisionProposal(
  ...
  const authorization = writingApplyAuthorizationSchema.parse(input.authorization);
  const stored = await project.store.getWritingApplyAuthorization(input.proposalId);
  if (
    stored === undefined ||
    hashJson(stored) !== hashJson(authorization) ||
  ...
  const result = await effects.execute<AppliedWritingRevision>(
    {
      intent: {
        effectId: `writing-application:${authorization.transactionId}`,
        ownerId: `writing-operation:${authorization.operationId}`,
        implementationId: WRITING_APPLY_AUTHORIZATION_POLICY_ID,
        parametersDigest: hashJson(authorization),
        recovery: {
          kind: 'buffered_mutation',
          authority: project.store.identity.projectId,
          reconcilerId: 'writing-agent.application-journal@1',
          transactionId: authorization.transactionId
        },
        exposure: NO_EFFECT_EXPOSURE
      },
      codec: { encode: parseJsonObject, decode: (value) => appliedWritingRevisionSchema.parse(value) },
  ...
  if (result.status !== 'settled')
    throw new Error(`Writing application requires reconciliation: ${result.status}.`);
  return result.observation;
}

async function applyAuthorizedRevision(
  project: WritingProject,
  input: {
    readonly proposalId: string;
    readonly authorization: WritingApplyAuthorization;
  ...
  const verification = view.productionVerifications.get(proposal.proposalId);
  if (verification === undefined)
    throw new Error(`Writing proposal production verification is unavailable: ${proposal.proposalId}`);
  ...
  if (
    ...
    hashJson(durableAuthorization) !== hashJson(input.authorization)
  ) {
  ...
  const existingRevision = findCommittedProposalRevision(
    view.records,
  ...
  if (proposalView.status === 'applied' && existingRevision !== undefined) {
    ...
    if (settlement === undefined)
      throw new Error(`Applied proposal has no durable mutation settlement: ${proposal.proposalId}`);
    return Object.freeze({
      proposalId: proposal.proposalId,
      revisionId: existingRevision.revision.revisionId,
      transactionId: settlement.transactionId,
      fileChanges: settlement.oldAndNewHashes,
      provenance: existingRevision.authorshipProvenance.filter(
  ...
  if (proposalView.status !== 'accepted')
    throw new Error(
      `Writing proposal must be accepted before application: ${proposal.proposalId} is ${proposalView.status}.`
    );
  if (view.current.revision.revisionId !== proposal.baseProjectRevisionId)
    throw new Error(`Writing proposal base is stale: ${proposal.proposalId}`);
  const operation = view.operations.get(proposal.operationId);
  ...
  try {
    const prior = await journal.withAuthority(project.authority, (authority) =>
      authority.receipt(transactionId)
    );
    if (prior !== undefined) {
      ...
      const observation = await editText(
        {
          files: textEdits.map((request) => ({
            ...
            edits: request.edits.map((edit) => ({
              range: edit.range,
              expectedText: expectedAnchorText(plan, request.resourceId, edit),
        ...
        },
        {
          ...
          services: {
            rootedFileAuthority: project.authority,
            patchJournal: journal,
            localToolConfiguration: DEFAULT_LOCAL_TOOL_CONFIGURATION
          }
        }
      );
      if (observation.kind !== 'result' || !observation.ok)
        throw new Error(`Agent Core edit_text rejected proposal application: ${observation.summary}`);
      const output = editTextOutputSchema.parse(observation.output);
      if (output.applicationStatus !== 'applied' && output.applicationStatus !== 'no_change')
        throw new Error(
          `Agent Core edit_text did not establish a committed result: ${output.applicationStatus}`
        );
      const receipt = await journal.withAuthority(project.authority, (authority) =>
        authority.receipt(transactionId)
      );
      if (receipt === undefined)
        throw new Error(`Committed text edit has no durable transaction receipt: ${transactionId}`);
  ...
  transactionResult ??= {
    outcome: 'committed',
    cleanup: { status: 'succeeded', diagnostics: [], strandedPaths: [] }
  };
  if (transactionResult.outcome === 'rolled_back')
    throw new Error(`Writing text transaction rolled back: ${transactionResult.failure.message}`);
  if (transactionResult.outcome === 'rollback_failed')
    throw new Error(`Writing text transaction rollback is uncertain: ${transactionResult.failure.message}`);
  await assertCommittedFiles(project, plan);
  for (const item of plan.values()) {
    await project.store.putObject(item.oldContent);
    await project.store.putObject(item.newContent);
  }
  ...
  if (view.current.revision.revisionId !== proposal.baseProjectRevisionId) {
    ...
    if (completed !== undefined)
      return Object.freeze({
        ...
        transactionId,
        fileChanges: view.settlements.get(proposal.proposalId)?.oldAndNewHashes ?? [],
        provenance: completed.authorshipProvenance.filter(
          (record) => record.proposalId === proposal.proposalId
        ),
        recoveredFinalization: true
      });
    throw new Error(
      'Project revision changed after the text transaction committed; finalization requires explicit reconciliation.'
    );
  }
  const resources = view.current.resources.map((resource) => {
    ...
    if (request === undefined)
      throw new Error(`Committed text plan has no localized edit request: ${resource.resourceId}`);
    return {
      ...resource,
      currentSha256: item.newSha256,
      protectedRanges: rebaseProtectedRanges(item.oldContent, request, resource.protectedRanges)
    };
  });
  const structural = applyStructuralChanges(view.current, proposal.structuralChanges);
  const proposedRecords = proposal.proposedAuthorshipProvenance.map((record) =>
    authorshipProvenanceSchema.parse(record)
  );
  const acceptedRecords = proposedRecords
  ...
  const provisional = createProjectRevision({
    ...
    editorialFindingIds: [
      ...view.current.revision.editorialFindingIds,
    ...
    editorialDecisions: [
      ...view.current.editorialDecisions.filter(
        (decision) => decision.decisionId !== acceptanceDecision.decisionId
      ),
      acceptanceDecision
    ]
  });
  const committedProvenance = allNewProvenance.map((record) => ({
    ...record,
    projectRevisionId: provisional.revision.revisionId
  }));
  ...
  const settlement: ProjectMutationSettlement = {
    ...
    changedPaths: fileChanges
      .filter((change) => change.oldSha256 !== change.newSha256)
      .map((change) => change.path),
    addedPaths: [],
    deletedPaths: [],
    cleanup: transactionResult.cleanup.status,
    remainingUncertainty:
      transactionResult.outcome === 'committed_with_residue'
        ? ['Agent Core transaction committed with cleanup residue.']
        : []
  };
  ...
  return Object.freeze({
    ...
    transactionId,
    fileChanges,
    provenance: Object.freeze(committedProvenance),
...
function assertWritingApplyAuthorization(
  ...
  if (
    parsed.authorizationId !== contentId('writing-apply-authorization', expected) ||
    hashJson({
...
export async function undoWritingRevision(
  project: WritingProject,
  input: {
    readonly revisionId?: string;
  ...
  if (restoredRevisionId === undefined)
    throw new Error('The current project revision has no parent content to restore.');
  const targetRecord = view.records.find(
    (record) =>
      record.payload.kind === 'revision.committed' &&
      record.payload.snapshot.revision.revisionId === restoredRevisionId
  );
  if (targetRecord?.payload.kind !== 'revision.committed')
    throw new Error(`Unknown project revision to restore: ${restoredRevisionId}`);
  ...
  if (requested !== undefined && requested.size !== input.resourceIds?.length)
    throw new Error('Undo resource IDs must be unique.');
  ...
  const plan = new Map<
    string,
    { path: string; oldContent: string; newContent: string; oldSha256: string; newSha256: string }
  >();
  for (const currentResource of selected) {
    const priorResource = target.resources.find(
      (resource) => resource.resourceId === currentResource.resourceId
    );
    if (priorResource === undefined) {
      if (requested?.has(currentResource.resourceId))
        throw new Error(
          `Selected resource did not exist at revision ${restoredRevisionId}: ${currentResource.resourceId}`
        );
      continue;
    }
    if (currentResource.currentSha256 === priorResource.currentSha256) continue;
    const currentFile = await readRootedText(
      project.authority,
      currentResource.relativePath,
      64 * 1024 * 1024
    );
    if (currentFile.sha256 !== currentResource.currentSha256)
      throw new Error(`Undo preimage is stale for resource: ${currentResource.resourceId}`);
  ...
  if (plan.size === 0)
    throw new Error(`No selected current resource differs from revision ${restoredRevisionId}.`);
  const explanation = input.explanation.trim();
  ...
  const mutationId = contentId('undo', {
    ...
    restoredRevisionId,
    resources: [...plan].map(([resourceId, item]) => ({
      resourceId,
      oldSha256: item.oldSha256,
      newSha256: item.newSha256
    }))
  ...
  try {
    ...
    if (prior !== undefined) {
      ...
      const observation = await editText(
        {
          files: [...plan].map(([, item]) => ({
            ...
            edits: [
              {
                range: completeTextRange(item.oldContent),
                expectedText: item.oldContent,
                replacementText: item.newContent
              }
            ]
          })),
          dryRun: false,
        ...
        },
        {
          ...
          invocation: {
            runId: operationId,
            turnId: `undo-${mutationId}`,
            requestAttempt: 1,
            toolBatchId: `undo-${mutationId}`,
            callIndex: 0,
            toolAttempt: 1
          }
        }
      ...
      if (observation.kind !== 'result' || !observation.ok)
        throw new Error(`Agent Core edit_text rejected undo: ${observation.summary}`);
      const output = editTextOutputSchema.parse(observation.output);
      if (output.applicationStatus !== 'applied' && output.applicationStatus !== 'no_change')
        throw new Error(`Agent Core edit_text did not establish undo: ${output.applicationStatus}`);
      const receipt = await journal.withAuthority(project.authority, (authority) =>
        authority.receipt(transactionId)
      );
      if (receipt === undefined)
        throw new Error(`Committed undo has no durable transaction receipt: ${transactionId}`);
      transactionResult = receipt.result;
    }
  } finally {
    journal.close();
  ...
  if (transactionResult.outcome === 'rolled_back')
    throw new Error(`Writing undo transaction rolled back: ${transactionResult.failure.message}`);
  if (transactionResult.outcome === 'rollback_failed')
    throw new Error(`Writing undo rollback is uncertain: ${transactionResult.failure.message}`);
  await assertCommittedFiles(project, plan);
  for (const item of plan.values()) {
    await project.store.putObject(item.oldContent);
    await project.store.putObject(item.newContent);
  ...
  if (refreshed.current.revision.revisionId !== view.current.revision.revisionId)
    throw new Error('Project revision changed while undo was being finalized.');
  const resources = refreshed.current.resources.map((resource) => {
    ...
    if (restored === undefined)
      throw new Error(`Undo target resource metadata is unavailable: ${resource.resourceId}`);
  ...
  const provisionalProvenance = [...plan].map(([resourceId, item]) =>
    authorshipProvenanceSchema.parse({
      ...
      supersedesProvenanceIds: refreshed.current.authorshipProvenance
  ...
  const provenance = provisionalProvenance.map((record) =>
    authorshipProvenanceSchema.parse({ ...record, projectRevisionId: provisional.revision.revisionId })
  );
  const snapshot = createProjectRevision({
    ...snapshotParts(provisional),
    parentRevisionIds: [refreshed.current.revision.revisionId],
  ...
  const settlement: ProjectMutationSettlement = {
    ...
    remainingUncertainty:
      transactionResult.outcome === 'committed_with_residue'
        ? ['Agent Core undo transaction committed with cleanup residue.']
        : []
  };
...
async function planTextTransaction(
  project: WritingProject,
  ...
  for (const request of edits) {
    ...
    if (resource.currentSha256 !== request.baseSha256)
      throw new Error(`Proposal resource hash is stale: ${request.resourceId}`);
    ...
    if (file.sha256 !== request.baseSha256) {
      ...
      if (file.sha256 === textSha256(newContent)) {
        plan.set(request.resourceId, {
          path: resource.relativePath,
          oldContent,
          newContent,
          oldSha256: request.baseSha256,
    ...
    plan.set(request.resourceId, {
      ...
      newSha256: textSha256(newContent)
    });
  }
  return plan;
}
...
function assertRecoveryPlan(
  recovery: z.infer<typeof editTextRecoveryPayloadSchema>,
  plan: ReadonlyMap<string, { path: string; oldSha256: string; newSha256: string }>,
  transactionId: string
): void {
  if (recovery.transactionId !== transactionId)
    throw new Error('Recovered text transaction ID does not match proposal application.');
  for (const item of plan.values()) {
    const file = recovery.files.find((candidate) => candidate.path === item.path);
    if (file?.oldSha256 !== item.oldSha256 || file.newSha256 !== item.newSha256)
      throw new Error(`Recovered text transaction does not match the staged proposal file: ${item.path}`);
  }
}

async function assertCommittedFiles(
  project: WritingProject,
  plan: ReadonlyMap<string, { path: string; newSha256: string }>
): Promise<void> {
  for (const item of plan.values()) {
...
function expectedAnchorText(
  ...
  if (content === undefined)
    throw new Error(`Text application plan lacks anchor preimage content: ${resourceId}`);
  const offsets = offsetRange(content, edit.range);
  const expectedText = content.slice(offsets.start, offsets.end);
  if (textSha256(expectedText) !== edit.expectedTextSha256)
    throw new Error(`Text application plan anchor preimage is stale: ${edit.anchorId}`);
  return expectedText;
}
...
): { readonly nodes: ProjectSnapshot['nodes']; readonly relations: ProjectSnapshot['relations'] } {
  let nodes = [...base.nodes];
  let relations = [...base.relations];
  for (const change of changes) {
    if (change.kind === 'create') {
      const value = z.strictObject({ node: documentNodeSchema }).parse(change.value);
      if (nodes.some((node) => node.nodeId === value.node.nodeId))
        throw new Error(`Structural create reuses a node ID: ${value.node.nodeId}`);
      if (
        value.node.parentId !== null &&
        !nodes.some((node) => node.nodeId === value.node.parentId && node.status !== 'removed')
      )
        throw new Error(`Structural create has an unknown parent: ${value.node.parentId}`);
      nodes.push(value.node);
    } else if (change.kind === 'remove') {
      const targets = new Set(change.targetIds);
      nodes = nodes.map((node) =>
        targets.has(node.nodeId) ? { ...node, status: 'removed' as const } : node
      );
      ...
      nodes = nodes.map((node) =>
        orders.has(node.nodeId)
          ? { ...node, siblingOrder: orders.get(node.nodeId) ?? node.siblingOrder }
          : node
      );
      ...
      if (value.action === 'add') {
        if (
          !nodes.some((node) => node.nodeId === value.relation.sourceId) ||
          !nodes.some((node) => node.nodeId === value.relation.targetId)
        )
          throw new Error(`Relation endpoints are not current project nodes: ${value.relation.relationId}`);
        relations.push(value.relation);
      } else
        relations = relations.map((relation) =>
          relation.relationId === value.relationId ? { ...relation, status: 'removed' as const } : relation
        );
    } else {
      const value = z
        .strictObject({ replacementNodes: z.array(documentNodeSchema).min(1) })
        .parse(change.value);
      const targets = new Set(change.targetIds);
      nodes = [
        ...nodes.map((node) => (targets.has(node.nodeId) ? { ...node, status: 'removed' as const } : node)),
        ...value.replacementNodes
      ];
    }
  }
  validateTree(nodes);
...
function validateTree(nodes: readonly ProjectSnapshot['nodes'][number][]): void {
  ...
  for (const node of active)
    if (node.parentId !== null && !ids.has(node.parentId))
      throw new Error(`Document node has an unknown active parent: ${node.nodeId}`);
  for (const node of active) {
...
function requireProposalDecision(
  records: readonly import('./project-store.js').ProjectLogRecord[],
  proposalId: string,
  ...
  const record = [...records]
    ...
    .find(
      (candidate) =>
        candidate.payload.kind === 'editorial.decision' &&
        candidate.payload.decision.proposalId === proposalId &&
        candidate.payload.decision.decision === decision
    );
  if (record?.payload.kind !== 'editorial.decision')
    throw new Error(`Proposal has no durable ${decision} editorial decision: ${proposalId}`);
  return record.payload.decision;
}

function findCommittedProposalRevision(
  records: readonly import('./project-store.js').ProjectLogRecord[],
  operationId: string,
  proposalId: string
  ...
  const settled = records.some(
    (record) =>
      record.payload.kind === 'mutation.settled' && record.payload.settlement.mutationId === proposalId
  );
  ...
  const record = [...records]
    ...
    .find(
      (candidate) =>
        candidate.payload.kind === 'revision.committed' &&
        candidate.payload.snapshot.revision.operationId === operationId
    );
