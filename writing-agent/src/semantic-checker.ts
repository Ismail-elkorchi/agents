      ...
      items: Object.freeze({
        ...
        properties: Object.freeze({
          ...
          coverage: Object.freeze({
            type: 'string',
            enum: Object.freeze(['complete', 'partial', 'unknown'])
          }),
...
export function createDefaultWritingEditorialChecker(input: {
  ...
  const provider = input.inference.options.provider;
  const identity = createHash('sha256')
    .update(
      JSON.stringify({
        ...
        contract: 'writing-agent.semantic-editorial-model-check@3'
      })
    )
    .digest('hex');
  const implementationId = `writing-agent.semantic-editorial-model-check@3:${identity}`;
  const verificationPolicyId = 'writing-agent.semantic-editorial-verification@3';
  return Object.freeze({
    ...
    async verify(verification: Parameters<WritingEditorialChecker['verify']>[0]) {
      ...
      if (response.terminationReason !== 'stop')
        throw new Error(`Semantic verifier did not complete normally: ${response.terminationReason}.`);
      const parsed = modelVerificationSchema.parse(JSON.parse(response.content));
      exactSet(
        parsed.semantic.map((item) => item.scope),
        semanticScopes,
        'semantic scopes'
      );
      exactSet(
        parsed.editorial.map((item) => item.criterionId),
        editorialCriteria.map((criterion) => criterion.criterionId),
        'editorial criteria'
      );
      const intendedChanges =
        verification.declaration.kind === 'changes'
          ? verification.declaration.items.map((item) => item.itemId)
          : [];
      const intents = new Map(
        verification.operationContract.intents.map((intent) => [intent.intentId, intent])
      );
      const semanticPreservationFindings: SemanticPreservationFinding[] = parsed.semantic.map((item) => {
        ...
        if (intent === undefined)
          throw new Error(`Semantic verifier returned an unrequested intent scope: ${item.scope}`);
        return semanticPreservationFindingSchema.parse({
          findingId: contentId('semantic-finding', {
          ...
          supportingCitations: resolveCitations(
            item.citationIds,
            verification.citationCatalog,
            new Set(intent.targetResourceIds)
          ),
          intendedChanges,
      ...
      const editorialFindings: EditorialFinding[] = parsed.editorial.map((item) => {
        ...
        if (criterion === undefined)
          throw new Error(`Semantic verifier returned an unrequested criterion: ${item.criterionId}`);
        return editorialFindingSchema.parse({
          findingId: contentId('editorial-finding', {
            operationId: verification.operation.operationId,
      ...
      return Object.freeze({
        semanticPreservationFindings: Object.freeze(semanticPreservationFindings),
        editorialFindings: Object.freeze(editorialFindings)
      });
    }
  });
}
...
function verificationPayload(
  ...
  const targetIds = new Set(
    verification.operationContract.targets.resources.map((resource) => resource.resourceId)
  );
  const baselines = verification.comparisonBaselines.map((baseline) =>
    Object.freeze({
  ...
  const payload = {
    contract: 'writing-agent.semantic-editorial-verification@3',
    verificationInputSha256: verification.verificationInputSha256,
    deliveredContextSelection: verification.contextSelection,
    semanticScopes,
...
function resolveCitations(
  ...
  if (new Set(citationIds).size !== citationIds.length)
    throw new Error('Semantic verifier repeated a citation identity.');
  ...
  return Object.freeze(
    citationIds.map((citationId) => {
      ...
      if (citation === undefined)
        throw new Error(`Semantic verifier returned an unknown host-issued citation: ${citationId}`);
      if (
        targetResourceIds !== undefined &&
        citation.kind !== 'source' &&
        !targetResourceIds.has(citation.resourceId)
      ) {
        throw new Error(
          `Semantic verifier cited text outside the intent's admitted resources: ${citationId}`
        );
      }
      return citation;
    })
...
function evidenceExcerpts(
  ...
  return Object.freeze(
    verification.operationContract.evidenceRequirements.sources.flatMap((source) =>
      source.excerpts.map((excerpt) => {
        ...
        try {
          ...
          const valid =
            textSha256(text) === excerpt.textSha256 && textSha256(content) === excerpt.sourceRevisionSha256;
          return Object.freeze({
            sourceId: source.sourceId,
            excerptId: excerpt.excerptId,
            resourceId,
