...
export async function assembleProposalVerificationMaterial(
  input: ProposalVerificationInput
): Promise<ProposalVerificationMaterial> {
  const view = await input.project.store.view();
  const base = view.current;
  if (base.revision.revisionId !== input.operation.baseProjectRevisionId)
    throw new Error('Proposal targets a stale project revision.');
  if (base.brief.briefRevisionId !== input.operation.briefRevisionId)
    throw new Error('Proposal targets a stale brief revision.');
  if (
  ...
  assertIntentChangeBindings(
    input.operation,
    input.contextSelection,
    input.textEdits,
    input.structuralChanges
  );
  for (const resource of base.resources) {
    ...
    if (file.sha256 !== resource.currentSha256)
      throw new Error(`Managed resource changed before proposal verification: ${resource.resourceId}`);
  ...
  for (const request of input.textEdits) {
    if (!targetResources.has(request.resourceId))
      throw new Error(`Proposal expands beyond admitted resource targets: ${request.resourceId}`);
    if (resourceEdits.has(request.resourceId))
      throw new Error(`Proposal repeats a resource edit group: ${request.resourceId}`);
    const resource = base.resources.find((candidate) => candidate.resourceId === request.resourceId);
    if (resource === undefined)
      throw new Error(`Proposal targets an unknown resource: ${request.resourceId}`);
    if (request.baseSha256 !== resource.currentSha256)
      throw new Error(`Proposal base hash is stale for resource: ${request.resourceId}`);
    const currentContent = baseText.get(request.resourceId);
    if (currentContent === undefined || textSha256(currentContent) !== request.baseSha256)
      throw new Error(`Proposal preimage changed for resource: ${request.resourceId}`);
    ...
    for (const offset of applied.offsets) {
      ...
      if (edit === undefined)
        throw new Error(
          `Applied edit offset has no canonical intent binding: ${request.resourceId}/${offset.anchorId}`
        );
      proposedAuthorship.push(
        authorshipProvenanceSchema.parse({
          provenanceId: contentId('provenance', {
    ...
    proposedAuthorship.push(
      ...carryForwardProvenance({
        records: activeAuthorship.filter(
          (record) => record.resourceId === request.resourceId && record.range !== undefined
        ),
  ...
  for (const change of input.structuralChanges) {
    ...
    proposedAuthorship.push(
      authorshipProvenanceSchema.parse({
        provenanceId: contentId('provenance', {
          proposalId: input.proposalId,
          structuralChangeId: change.changeId
        }),
        projectRevisionId: base.revision.revisionId,
  ...
  if (resourceEdits.size === 0 && input.structuralChanges.length === 0)
    throw new Error('A revision proposal requires at least one text edit or structural change.');
  validateSemanticDeclaration(input.declaration, input.operation);
  const preservationContract = derivePreservationContract(
    base,
    input.operation,
    input.textEdits,
    input.structuralChanges,
  ...
  const comparisonBaselines = await loadComparisonBaselines(
    input.project,
    base,
    baseText,
    preservationContract.comparisonBaselineRevisionIds,
    view.records
  );
...
export async function runDeterministicProposalVerification(
  input: ProposalVerificationInput
): Promise<ProposalDeterministicVerification> {
...
export async function verifyProposalProduction(input: {
  ...
  const criterionCoverage = acceptanceCriterionCoverage(
    verificationMaterial.base,
    verificationMaterial.deterministicChecks,
...
export function proposalSatisfiesRequiredVerification(
  verification: {
    readonly deterministicChecks: readonly DeterministicCheck[];
    readonly semanticPreservationFindings: readonly SemanticPreservationFinding[];
    readonly editorialFindings: readonly EditorialFinding[];
  ...
  humanCriterionDecisions: readonly HumanCriterionDecision[] = []
  ...
  if (decisions.size !== humanCriterionDecisions.length)
    throw new Error('Human acceptance criterion decisions must be unique.');
  const reasons: string[] = [];
  for (const check of verification.deterministicChecks) {
    if (check.requirement === 'required' && check.verdict !== 'passed')
      reasons.push(`check:${check.checkId}:${check.verdict}`);
  }
  ...
  for (const coverage of verification.criterionCoverage) {
    ...
    if (coverage.verificationKind === 'human') {
      ...
      if (decision?.verdict !== 'passed')
        reasons.push(`criterion:${coverage.criterionId}:${decision?.verdict ?? 'missing'}/human`);
...
function derivePreservationContract(
  ...
  proposals: ReadonlyMap<
    string,
    { readonly proposal: { readonly operationId: string }; readonly status: string }
  >,
  ...
  const acceptedRevisionIds = records.flatMap((record) =>
    record.payload.kind === 'revision.committed' &&
  ...
  return preservationContractSchema.parse({
    ...
    protectedResourceHashes: Object.fromEntries(
      base.resources
    ...
    protectedRangeIds: base.resources.flatMap((resource) =>
      resource.protectedRanges.map((range) => range.rangeId)
    ),
    protectedCriterionIds: base.brief.acceptanceCriteria
      .filter((criterion) => !affectedCriteria.has(criterion.criterionId))
      .map((criterion) => criterion.criterionId),
    protectedClaimIds: base.claims
      .filter((claim) => !affectedClaims.has(claim.claimId))
      .map((claim) => claim.claimId),
    protectedEvidenceRelationIds: base.evidenceRelations
      .filter((relation) => !affectedRelations.has(relation.relationId))
      .map((relation) => relation.relationId),
    protectedEditorialDecisionIds: base.editorialDecisions
      .filter((decision) => !affectedDecisions.has(decision.decisionId))
      .map((decision) => decision.decisionId),
    priorAcceptedProposalIds: [...proposals]
      .filter(([, view]) => view.status === 'accepted' || view.status === 'applied')
    ...
    stableSemanticScopes: [
      'all content outside admitted intent targets',
      'prior accepted claims, evidence relations, and editorial decisions'
    ],
    comparisonBaselineRevisionIds: [...new Set([...acceptedRevisionIds, base.revision.revisionId])].sort(),
    requiredRevalidations: [
      'project-base',
      'resource-base',
...
function deterministicProposalChecks(input: {
  ...
  const add = (
    ...
    checks.push(
      deterministicCheckSchema.parse({
        checkId,
        implementationId: `writing-agent.check.${checkId}@2`,
        criterionIds,
        requirement,
        verdict,
        summary,
  ...
  add(
    'project-base',
    'required',
    input.operation.baseProjectRevisionId === input.base.revision.revisionId ? 'passed' : 'failed',
    'Proposal derives from the expected project revision.'
  );
  const hashesValid = input.textEdits.every(
    (request) =>
      input.base.resources.find((resource) => resource.resourceId === request.resourceId)?.currentSha256 ===
      request.baseSha256
  );
  add(
    'resource-base',
    'required',
    hashesValid ? 'passed' : 'failed',
    'Every localized edit received its current managed-resource hash from application-owned target control.'
  );
  const confined =
  ...
  add(
    'mutation-confinement',
    'required',
    confined ? 'passed' : 'failed',
    'Proposed changes remain inside the admitted target union.'
  );
  add(
    'protected-ranges',
    'required',
    'passed',
    'Protected exact ranges were checked against localized edits before proposal creation.'
  );
  add(
    'semantic-declaration',
    'required',
    semanticDeclarationConfined(input.declaration, input.operation) ? 'passed' : 'failed',
    'Declared semantic changes are confined to admitted intents.'
  );
  add(
    'claim-evidence-graph',
    'required',
    graphIntegrity(input.base) ? 'passed' : 'failed',
    'Claim, evidence, source, and excerpt references are internally consistent.'
  );
  add(
    'source-record-integrity',
    'required',
    sourceRecordIntegrity(input.base, input.baseText) ? 'passed' : 'failed',
    'Local source identity, excerpt ranges, quotation hashes, and supplied verifier bindings are internally consistent.'
  );
  add(
    'prior-decisions',
    'required',
    'passed',
    'Protected prior decisions and out-of-scope resources remain in the preservation contract.'
  );
  add(
    'provenance-graph',
    'required',
    provenanceIntegrity(input.base, input.baseText) ? 'passed' : 'failed',
    'Authorship-provenance targets and exact range coverage are valid for current resources and structural nodes.'
  );
  for (const constraint of input.operation.effectiveConstraints.lengthConstraints) {
    ...
    const passed =
      actual >= (constraint.minimum ?? 0) && actual <= (constraint.maximum ?? Number.MAX_SAFE_INTEGER);
    add(
      `length-${constraint.constraintId}`,
      constraint.requirement,
      passed ? 'passed' : 'failed',
      'The effective operation length intersection was evaluated against proposed text.',
      [
  ...
  for (const constraint of input.operation.effectiveConstraints.exactConstraints) {
    ...
    const allowed = new Set(
      constraint.allowedValues.map((value) => normalizeExactValue(value, constraint.matcher))
    );
    if (constraint.baselinePolicy === 'include')
      for (const value of extractExactValues(baseline, constraint.matcher)) allowed.add(value);
    const unexpected = proposedValues.filter((value) => !allowed.has(value));
    add(
      `exact-${constraint.constraintId}`,
      constraint.requirement,
      unexpected.length === 0 ? 'passed' : 'failed',
      `The ${constraint.matcher} closed world was checked against its explicit allowlist and baseline policy.`,
      [
  ...
  const exclusionResults = input.base.brief.excludedContent.map(
    (constraint) => !combined.includes(constraint.statement)
  );
  add(
    'excluded-content',
    'advisory',
    exclusionResults.length === 0 ? 'passed' : exclusionResults.every(Boolean) ? 'passed' : 'failed',
    'Literal excluded-content constraints were checked.'
  );
  ...
  add(
    ...
    'advisory',
    terminology,
    'Explicit require:/forbid: terminology constraints were checked; unsupported natural-language constraints remain unknown.'
  );
  add(
    'heading-hierarchy',
    'advisory',
    headingVerdict(input.base, input.proposedText),
    'Markdown heading levels do not skip hierarchy levels in affected resources.'
  );
  add(
    'internal-references',
    'advisory',
    internalReferenceVerdict(input.base, combined),
    'Structured source references resolve to current project source records.'
  );
  add(
    'source-identity',
    'advisory',
    input.base.sources.some(
      (source) => source.identityStatus === 'conflicting' || source.identityStatus === 'unavailable'
    )
      ? 'failed'
      : 'passed',
    'Recorded source identity states contain no hidden success conversion.'
  );
  add(
    'duplicate-passages',
    'advisory',
    duplicatePassageVerdict(combined),
    'Exact duplicate passages above 240 characters were checked.'
  );
  add(
    'syntax',
    'advisory',
    syntaxVerdict(input.base, input.proposedText),
    'Supported Markdown fence syntax was checked.'
  );
...
export function acceptanceCriterionCoverage(
  ...
  return Object.freeze(
    base.brief.acceptanceCriteria.map((criterion) => {
      ...
      if (criterion.verificationKind === 'deterministic') {
        ...
        return criterionCoverageSchema.parse({
          ...
          explanation:
            selected.length === 0
              ? 'No deterministic checker declared coverage of this criterion.'
              : 'Coverage is derived only from deterministic checks that explicitly bind this criterion ID.'
        });
      }
      ...
      const coverage =
        selected.length === 0
          ? 'none'
          : selected.every((finding) => finding.coverage === 'complete')
            ? 'complete'
            : 'partial';
      return criterionCoverageSchema.parse({
        criterionId: criterion.criterionId,
        ...
        explanation:
          selected.length === 0
            ? 'No admitted editorial evaluator finding declared coverage of this criterion.'
            : 'Coverage is derived only from editorial findings that explicitly bind this criterion ID.'
      });
    })
  );
}
...
function aggregateVerdict(
  verdicts: readonly ('passed' | 'failed' | 'unknown')[]
...
function assertProtectedRanges(
  content: string,
  protectedRanges: ProjectSnapshot['resources'][number]['protectedRanges'],
  request: LocalizedTextEdit,
  operation: WritingOperation
): void {
  ...
  for (const protectedRange of protectedRanges) {
    ...
    for (const edit of request.edits) {
      ...
      const exactDecision = operation.intents.some(
        (intent) =>
          intent.targetRangeIds.includes(protectedRange.rangeId) &&
          intent.affectedEditorialDecisionIds.length > 0
      );
      if (protectedRange.decisionRequired && !exactDecision)
        throw new Error(
          `Proposal changes protected range without its required decision: ${protectedRange.rangeId}`
        );
      if (!operation.intents.some((intent) => intent.targetRangeIds.includes(protectedRange.rangeId)))
        throw new Error(
          `Proposal changes a protected range outside the admitted exact targets: ${protectedRange.rangeId}`
        );
    }
  }
}

function coveringProvenance(
  base: ProjectSnapshot,
  resourceId: string,
  content: string,
  request: LocalizedTextEdit
  ...
  return Object.freeze(
    activeProvenance(base.authorshipProvenance)
      .filter((record) => {
    ...
    for (const [index, range] of ranges.entries()) {
      records.push(
        authorshipProvenanceSchema.parse({
          provenanceId: contentId('provenance', {
            proposalId: input.proposalId,
            carriedFrom: prior.provenanceId,
            index,
            range
          }),
...
function assertIntentChangeBindings(
  ...
  const assertOrdered = (intentIds: readonly string[], label: string): void => {
    if (new Set(intentIds).size !== intentIds.length)
      throw new Error(`Canonical change repeats an intent identity: ${label}`);
  ...
  for (const request of textEdits) {
    const descriptor = contextSelection.targetDescriptors.find(
      (candidate) => candidate.resourceId === request.resourceId
    );
    if (descriptor === undefined)
      throw new Error(
        `Canonical text change has no application-owned target descriptor: ${request.resourceId}`
      );
    for (const edit of request.edits) {
      ...
      for (const intentId of edit.intentIds) {
        ...
        if (
          !intent?.targetResourceIds.includes(request.resourceId) ||
          (intent.targetRangeIds.length > 0 &&
            (anchor.targetRangeId === undefined || !intent.targetRangeIds.includes(anchor.targetRangeId)))
        ) {
          throw new Error(
            `Canonical text change is not authorized by its admitted intent: ${request.resourceId}/${edit.anchorId}/${intentId}`
          );
        }
      }
    }
  ...
  for (const change of structuralChanges) {
    ...
    for (const intentId of change.intentIds) {
      ...
      if (
        intent === undefined ||
        structuralKindForIntent(intent.kind) !== change.kind ||
        (change.kind !== 'create' &&
          change.targetIds.some((targetId) => !intent.targetNodeIds.includes(targetId)))
      ) {
        throw new Error(
          `Canonical structural change is not authorized by its admitted intent: ${change.changeId}/${intentId}`
        );
      }
    }
  }
...
function structuralKindForIntent(kind: string): StructuralChange['kind'] | undefined {
  return kind.startsWith('structure.')
    ? (kind.slice('structure.'.length) as StructuralChange['kind'])
    : undefined;
}
...
function survivingProvenanceSegments(
  ...
  const relevant = edits.filter(
    (edit) =>
      (edit.start < original.end && edit.end > original.start) ||
  ...
  if (oldCursor < original.end)
    segments.push({ start: newCursor, end: newCursor + (original.end - oldCursor) });
...
function validateStructuralChange(
  change: StructuralChange,
  base: ProjectSnapshot,
  operation: WritingOperation,
  targetNodes: ReadonlySet<string>
): void {
  if (change.kind === 'create') {
    const value = z.strictObject({ node: documentNodeSchema }).parse(change.value);
    if (
      change.targetIds.length !== 1 ||
      change.targetIds[0] !== value.node.nodeId ||
      !targetNodes.has(value.node.nodeId)
    )
      throw new Error(`Structural create expands beyond its admitted node identity: ${value.node.nodeId}`);
    return;
  }
  if (change.kind === 'relation') {
    ...
    if (value.action === 'add') {
      ...
      change.targetIds.length !== 1 ||
      change.targetIds[0] !== value.relationId ||
      !operation.intents.some((intent) => intent.affectedRelationIds.includes(value.relationId))
    ) {
      throw new Error(
        `Structural relation removal expands beyond admitted relation identity: ${value.relationId}`
      );
    }
  } else {
  ...
  if (
    !operation.intents.some(
      (intent) =>
        intent.kind === `structure.${change.kind}` ||
        (change.kind === 'relation' && intent.kind === 'structure.relation')
    )
  ) {
...
function validateSemanticDeclaration(
  declaration: SemanticChangeDeclaration,
  operation: WritingOperation
): void {
  if (!semanticDeclarationConfined(declaration, operation))
    throw new Error('Semantic-change declaration expands beyond the admitted intent graph.');
}

function semanticDeclarationConfined(
  declaration: SemanticChangeDeclaration,
  operation: WritingOperation
): boolean {
  if (declaration.kind === 'none') return true;
  const affected = new Set(
    operation.intents.flatMap((intent) => [
...
function graphIntegrity(base: ProjectSnapshot): boolean {
  ...
  return base.evidenceRelations.every((relation) => {
    ...
    return (
      claims.has(`${relation.claimId}@${String(relation.claimVersion)}`) &&
      source?.excerpts.some(
        (excerpt) =>
          excerpt.excerptId === relation.excerptId &&
          excerpt.sourceRevisionSha256 === relation.sourceRevisionSha256 &&
          excerpt.rangeSha256 === relation.rangeSha256
      ) === true
    );
  });
}

function sourceRecordIntegrity(
  base: ProjectSnapshot,
  ...
  return base.sources.every((source) => {
    if (
      source.identityStatus === 'verified' &&
      (source.authoritativeIdentifiers.length === 0 ||
        source.identityVerifierId === undefined ||
        source.verificationPolicyId === undefined)
    )
      return false;
    ...
    if (
      resource === undefined ||
      content === undefined ||
      resource.currentSha256 !== source.exactSha256 ||
      textSha256(content) !== source.exactSha256
    )
      return false;
...
function provenanceIntegrity(base: ProjectSnapshot, contentByResource: ReadonlyMap<string, string>): boolean {
  ...
  for (const record of base.authorshipProvenance) {
    if (
      known.has(record.provenanceId) ||
      record.supersedesProvenanceIds.some((provenanceId) => !known.has(provenanceId))
    )
      return false;
  ...
  if (
    !active.every((record) => {
      if (record.resourceId === undefined)
        return nodes.has(record.nodeId ?? '') && record.structuralObjectId !== undefined;
      const content = contentByResource.get(record.resourceId);
      if (!resources.has(record.resourceId) || content === undefined || record.range === undefined)
        return false;
      try {
        offsetRange(content, record.range);
        return true;
      } catch {
        return false;
      }
  ...
  for (const node of base.nodes.filter((candidate) => candidate.status !== 'removed')) {
    if (!active.some((record) => record.nodeId === node.nodeId && record.structuralObjectId !== undefined))
      return false;
  }
  for (const resource of base.resources) {
    ...
    const ranges = active
      .flatMap((record) =>
        record.resourceId === resource.resourceId && record.range !== undefined
          ? [offsetRange(content, record.range)]
          : []
      )
  ...
  return [...targetResourceIds]
    ...
    .map((resourceId) => {
      ...
      if (content === undefined)
        throw new Error(`Effective constraint targets unavailable resource text: ${resourceId}`);
      return content;
    })
    .join('\n');
}

function extractExactValues(
  content: string,
  matcher: 'number' | 'citation' | 'named-entity'
): readonly string[] {
  let values: readonly string[];
  if (matcher === 'number') {
    values =
      content.match(
        /(?<![\p{L}\p{N}_])(?:[$ۣ]\s*)?[+-]?(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?%?(?![\p{L}\p{N}_])/gu
      ) ?? [];
...
function normalizeExactValue(value: string, matcher: 'number' | 'citation' | 'named-entity'): string {
  ...
  if (matcher === 'citation' && /^https?:\/\//iu.test(compact)) {
    try {
      return new URL(compact).toString();
    } catch {
      return compact;
    }
  }
...
function terminologyVerdict(content: string, base: ProjectSnapshot): 'passed' | 'failed' | 'unknown' {
  ...
  for (const constraint of base.brief.terminologyConstraints) {
    if (constraint.statement.startsWith('require:')) {
      supported += 1;
      if (!content.includes(constraint.statement.slice(8).trim())) return 'failed';
    } else if (constraint.statement.startsWith('forbid:')) {
      supported += 1;
      if (content.includes(constraint.statement.slice(7).trim())) return 'failed';
    }
  }
...
function internalReferenceVerdict(base: ProjectSnapshot, content: string): 'passed' | 'failed' {
  ...
  return [...content.matchAll(/\[source:([A-Za-z0-9._:/-]+)\]/gu)].every((match) =>
    sources.has(match[1] ?? '')
  )
    ? 'passed'
    : 'failed';
...
async function loadComparisonBaselines(
  ...
  for (const revisionId of revisionIds) {
    ...
    if (snapshot === undefined)
      throw new Error(`Preservation baseline revision is unavailable: ${revisionId}`);
    ...
    for (const resource of snapshot.resources) {
      ...
      if (content === undefined || textSha256(content) !== resource.currentSha256)
        throw new Error(
          `Preservation baseline content is unavailable or corrupt: ${revisionId}/${resource.resourceId}`
        );
...
function assertFindingComparison(
  finding: Pick<
    SemanticPreservationFinding,
    'findingId' | 'baseRevisionId' | 'proposedRevisionId' | 'verificationInputSha256'
  >,
  input: {
    readonly base: ProjectSnapshot;
    readonly proposedRevisionId: string;
  ...
  if (
    finding.baseRevisionId !== input.base.revision.revisionId ||
    finding.proposedRevisionId !== input.proposedRevisionId ||
    finding.verificationInputSha256 !== input.verificationInputSha256
  ) {
    throw new Error(
      `Editorial checker returned a finding bound to different comparison inputs: ${finding.findingId}`
    );
  }
}
...
function assertFindingEvaluator(
  ...
  if (
    ...
    throw new Error(
      `Editorial checker returned a finding with mismatched implementation or calibration identity: ${finding.findingId}`
    );
  }
}
...
function assertSupportingCitations(
  ...
  if (finding.verdict !== 'unknown' && finding.supportingCitations.length === 0) {
    throw new Error(
      `A conclusive finding requires at least one exact host-issued citation: ${finding.findingId}`
    );
  }
  const catalog = new Map(citationCatalog.map((citation) => [citation.citationId, citation]));
  if (
    new Set(finding.supportingCitations.map((citation) => citation.citationId)).size !==
  ...
  if (finding.verdict !== 'unknown') {
    ...
    for (const kind of requiredKinds)
      if (!kinds.has(kind))
        throw new Error(`A conclusive finding lacks required ${kind} support: ${finding.findingId}`);
  }
}
...
function buildVerificationCitationCatalog(input: {
  ...
  for (const [resourceId, content] of [...input.proposedText]
  ...
  for (const baseline of input.comparisonBaselines) {
    for (const [resourceId, content] of [...baseline.text]
      .filter(([id]) => targetIds.has(id))
      .sort(([left], [right]) => left.localeCompare(right))) {
      for (const range of paragraphRanges(content)) {
        const offsets = offsetRange(content, range);
  ...
  for (const source of input.operationContract.evidenceRequirements.sources) {
    ...
    for (const excerpt of source.excerpts) {
      try {
        const offsets = offsetRange(content, excerpt.range);
        if (
          textSha256(content.slice(offsets.start, offsets.end)) !== excerpt.textSha256 ||
          excerpt.sourceRevisionSha256 !== source.exactSha256
        )
          continue;
        const material = {
