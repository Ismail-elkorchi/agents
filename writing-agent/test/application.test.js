...
test('review mode denies workspace mutations even when the model asks to patch a file', integration, async (t) => {
  const f = await fixture('Old line.\n', { mode: 'review', responses: [patchResponse(patch), 'Suggested revision: New line.'] });
  t.after(() => f.close());
  await f.application.start();
  await submit(f.application, 'Review the wording.');
  assert.equal(await readFile(path.join(f.root, 'document.txt'), 'utf8'), 'Old line.\n');
  const observation = (await f.application.readHistory()).entries.find((entry) => entry.type === 'observation');
  assert.equal(observation.ok, false);
  assert(!f.provider.requests[0].tools.some((tool) => tool.name === 'apply_patch'));
  assert(f.provider.requests[0].tools.some((tool) => tool.type === 'function' && tool.function.name === 'notes_write'));
});

test('conversation changes remain ordinary inputs across prompts and reopen, without fabricated brief requirements', integration, async (t) => {
  const prompts = ['Écris en français.', 'Actually write in Arabic, without the earlier length restriction.', 'Discuss a new topic without changing files.'];
  const f = await fixture('نص المستخدم\n', { responses: ['D’accord.', 'حسنًا.', 'A discussion.'] });
  let reopened;
  t.after(async () => { await reopened?.close(); await f.close(); });
  await f.application.start();
  for (const prompt of prompts) {
    const result = await submit(f.application, prompt);
    assert.equal(result.terminal.executionStatus, 'completed');
  }
  assert.deepEqual((await f.application.readHistory()).entries.filter((entry) => entry.type === 'input').map((entry) => entry.task), prompts);
  ...
  reopened = await openWritingApplication({ rootDirectory: f.root, stateRoot: f.stateRoot,
...
test('history and notes are available as model tools without preselected document excerpts', integration, async (t) => {
  const f = await fixture('Only retrieve this when needed.\n', { responses: [
    toolCall('read_files', { files: [{ path: 'document.txt', startLine: 1, lineCount: 10 }] }), 'Read.'
  ] });
  t.after(() => f.close());
  await f.application.start();
  ...
  for (const name of ['history_read', 'notes_write', 'context_transition'])
    assert(first.tools.some((tool) => tool.type === 'function' && tool.function.name === name), name);
});

test('the default Codex writing provider reaches inference without a wire output cap or recovery', integration, async (t) => {
  ...
  t.after(() => f.close());
  await app.start();
  const submission = await app.submit('Review document.txt.');
...
test('reopening a suspended review uses its recorded permission mode and never resends an unknown provider request', integration, async (t) => {
  const f = await fixture('Old line.\n', { mode: 'review', responses: [async () => { throw new Error('Connection lost.'); }] });
  let reopened;
  t.after(async () => { await reopened?.close(); await f.close(); });
  await f.application.start();
