...
test('stdio supports direct editing and subsequent read-only review in the same conversation', {
  ...
  t.after(async () => { await client.close(); await f.close(); });
  const first = await client.request('instruction.submit', { instruction: 'Replace Old line. with New line. in document.txt.' });
  ...
  const second = await client.request('instruction.submit', { instruction: 'Review the wording without editing.' });
  await client.notification('run.completed', ({ runId }) => runId === second.runId);
  assert.equal((await client.request('document.read', { path: 'document.txt' })).content, 'New line.\n');
  assert.equal((await client.request('history.read')).entries.filter((entry) => entry.type === 'input').length, 2);
