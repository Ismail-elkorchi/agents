...
for (const columns of [48, 120]) test(`writing TUI accepts a direct request without project setup at ${columns} columns`, {
  ...
  const unsubscribe = f.application.subscribe((event) => {
    if (event.type === 'run.completed') return runtime.dispatch({ type: 'result', result: event.result });
    if (event.type === 'run.progress') return runtime.dispatch({ type: 'progress', event: event.event });
    if (event.type === 'application.state.changed') return runtime.dispatch({ type: 'application', state: event.state });
  }, (error) => { throw error; });
  t.after(async () => { unsubscribe(); await runtime.dispose(); await f.close(); });
  await runtime.start();
  await waitForState(runtime, t.signal, () => runtime.state().history.length > 0);
  assert.equal(runtime.state().document, undefined);
  await runtime.dispatch({ type: 'composer.edit', transition: { kind: 'edit', operation: { kind: 'insert', text: 'Replace Old line. with New line. in document.txt.' } } });
...
test('the writing document viewer copies a selected passage into the visible composer', {
  skip: process.platform !== 'linux', timeout: 30_000
}, async (t) => {
  const f = await fixture('Opening.\nA passage.\n');
  const host = createMemoryTerminalHost({ terminalSize: { columns: 80, rows: 32 } });
  const runtime = createTuiRuntime({ host, app: createWritingAgentTuiApp(f.application) });
  t.after(async () => { await runtime.dispose(); await f.close(); });
  await runtime.start();
  await waitForState(runtime, t.signal, () => runtime.state().history.length > 0);
  await runtime.dispatch({ type: 'document.loaded', document: await f.application.readDocument('document.txt') });
  await runtime.dispatch({ type: 'document.toggle-source' });
  for (const operation of [{ kind: 'moveLineDown' }, { kind: 'moveHome' }, { kind: 'moveEnd', extendSelection: true }])
    await runtime.dispatch({ type: 'document.edit', transition: { kind: 'edit', operation } });
  await runtime.dispatch({ type: 'document.use-passage' });
  assert.match(textDocumentText(runtime.state().composer.document), /Passage from document\.txt:\n> A passage\./);
});

test('provider interruptions open recovery with the actual failure and preserve the next instruction', { skip: process.platform !== 'linux', timeout: 30_000 }, async (t) => {
  const f = await fixture('A short document.\n', { responses: [async () => { throw new Error('Provider connection closed'); }] });
